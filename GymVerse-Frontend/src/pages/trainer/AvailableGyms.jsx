...
const AvailableGyms = () => {
  ...
  const fetchApprovedGymIds = async () => {
    try {
      const res = await trainerService.getApprovedGymIds();
      setApprovedGymIds(res.data.data || []);
    } catch (error) {
      console.error('Failed to fetch approved gym IDs:', error);
    }
  };
  ...
  const getApplicationStatus = (gymId) => {
    if (approvedGymIds.includes(gymId)) {
      return { status: 'approved' };
    }
    const app = applications.find(a => a.gymId?._id === gymId);
    if (!app) return null;
    return { status: app.status, applicationId: app._id };
  };
            ...
            <div className="max-h-125 overflow-y-auto">
              {filteredGyms.length === 0 ? (
                ...
                filteredGyms.map((gym) => {
                  const appStatus = getApplicationStatus(gym._id);
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="ml-3">
                          {isApproved ? (
                            ...
                            <span className="px-3 py-1 rounded-full text-xs bg-yellow-500/20 text-yellow-400 flex items-center gap-1 whitespace-nowrap">
                            ...
                            <button
                              ...
                              className="px-3 py-1 bg-purple-600 rounded-lg text-white text-xs hover:bg-purple-700 flex items-center gap-1 whitespace-nowrap"
                            >
                              <ArrowPathIcon className="w-3 h-3" />
                              {processing === gym._id ? 'Applying...' : 'Apply Again'}
                            </button>
                          ) : (
                            <button
                              onClick={(e) => { e.stopPropagation(); handleApply(gym._id); }}
                            ...
                            >
                              ...
                              {processing === gym._id ? 'Applying...' : 'Apply'}
                            </button>
                          )}
                        </div>
                      </div>
