...
const GymDiscovery = () => {
  ...
  const [selectedGymForRating, setSelectedGymForRating] = useState(null);
  
  // Reviews Modal State - Sabhi users ke liye
  ...
  const fetchNearbyGyms = async (lng, lat) => {
    ...
    try {
      ...
      if (res.data.data?.length === 0) {
        setGyms([]);
        setFilteredGyms([]);
        toast.error('No gyms found near this location');
      } else {
        setGyms(res.data.data || []);
        setFilteredGyms(res.data.data || []);
      }
      ...
      setGyms([]);
      setFilteredGyms([]);
      toast.error('Failed to find gyms near this location');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getUserLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        setLocationError(true);
        setGettingLocation(false);
        toast.error('Unable to get your location. Please search by city name.');
      }
    );
  };
  ...
  const handleLocationSelect = async (loc) => {
    ...
    setLoading(true);
    try {
      const res = await gymService.getNearbyGyms(loc.lng, loc.lat, 50000);
      if (res.data.data?.length === 0) {
        toast.error(`No gyms found near "${loc.name.split(',')[0]}"`);
        setGyms([]);
        setFilteredGyms([]);
      } else {
        toast.success(`Found ${res.data.data.length} gyms near this location`);
        setGyms(res.data.data || []);
        setFilteredGyms(res.data.data || []);
      }
    } catch (err) {
  ...
  const handleReset = () => {
    ...
    setLocationError(false);
    fetchAllGyms();
    toast.success('Showing all gyms');
  };
  ...
  return (
    <>
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            {/* Rating Display - Sabhi Users Ke Liye */}
                            ...
                            <div className="flex gap-2 mt-3">
                              {/* Details Button - Sabhi users ke liye */}
                              <button
                                onClick={(e) => { e.stopPropagation(); handleViewDetails(gym); }}
                                className="flex-1 py-1.5 bg-blue-600 rounded-lg text-white text-xs hover:bg-blue-700 transition flex items-center justify-center gap-1"
                              >
                                <EyeIcon className="w-3 h-3" />
                                Details
                              </button>
                              
                              {/* Reviews Button - Sabhi users ke liye */}
                              <button
                                onClick={(e) => { 
                                  e.stopPropagation(); 
                                ...
                                className="px-3 py-1.5 bg-purple-600 rounded-lg text-white text-xs hover:bg-purple-700 transition flex items-center justify-center gap-1"
                              >
                                <EyeIcon className="w-3 h-3" />
                                Reviews
                              </button>
                              
                              {/* Rate Button - Sirf members ke liye / Buy Button - Non-members ke liye */}
                              {hasMembership ? (
                                <button
                                  onClick={(e) => { 
                                    e.stopPropagation(); 
                                    setSelectedGymForRating(gym);
                                ...
                                <button
                                  ...
                                  className="flex-1 py-1.5 bg-green-600 rounded-lg text-white text-xs hover:bg-green-700 transition flex items-center justify-center gap-1"
                                >
      ...
      {/* Rating Modal - Sirf members ke liye */}
      {showRatingModal && selectedGymForRating && (
        <RatingModal
          gym={selectedGymForRating}
          onClose={() => {
            setShowRatingModal(false);
            setSelectedGymForRating(null);
          }}
          onRatingSubmitted={handleRatingSubmitted}
        />
      )}

      {/* Reviews Modal - Sabhi users ke liye */}
