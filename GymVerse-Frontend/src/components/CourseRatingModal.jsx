...
const CourseRatingModal = ({ course, onClose, onRatingSubmitted, mode = 'reviews' }) => {
  ...
  const fetchReviews = async () => {
    ...
    try {
      const res = await courseService.getCourseRatings(course._id);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await courseService.addCourseRating(course._id, rating, review);
  ...
  // Reviews Only Mode (for Reviews button)
  if (mode === 'reviews') {
    return (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" onClick={onClose}>
        <div className="bg-gray-900 rounded-xl max-w-2xl w-full mx-4 border border-white/10 max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="p-6 border-b border-white/10 flex justify-between items-center shrink-0">
            <div>
              <h3 className="text-xl font-bold text-white">Course Reviews</h3>
              <p className="text-gray-400 text-sm mt-1">
                {course.title} • {totalReviews} {totalReviews === 1 ? 'review' : 'reviews'}
              </p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 min-h-0">
            <h4 className="text-white font-semibold mb-3">All Reviews ({totalReviews})</h4>
            {loadingReviews ? (
  ...
  // Rating/Update Mode (for Rate/Update button)
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-gray-900 rounded-xl max-w-md w-full mx-4 border border-white/10" onClick={(e) => e.stopPropagation()}>
        <div className="p-6">
          ...
          <div className="text-center mb-4">
            <p className="text-gray-300 mb-2">How would you rate {course.title}?</p>
