...
const GymMap = ({ gyms, onGymClick, userLocation, selectedGym }) => {  // ✅ Added selectedGym prop
  ...
  useEffect(() => {
    ...
    const defaultCenter = userLocation || [28.6139, 77.209];
  ...
  // ✅ FIXED: Add useEffect to center map on selected gym
  useEffect(() => {
    if (mapRef.current && selectedGym) {
      const lat = selectedGym.location?.coordinates?.[1];
      const lng = selectedGym.location?.coordinates?.[0];
      if (lat && lng) {
        mapRef.current.setView([lat, lng], 15);
        // Open popup for selected gym
  ...
  }, [selectedGym]); // ✅ Removed mapRef.current from dependency array
  ...
  return (
    <div
      ref={mapContainerRef}
      className="w-full h-125 rounded-xl bg-gray-800"
    />
  );
};
