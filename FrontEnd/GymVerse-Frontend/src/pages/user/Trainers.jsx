...
const Trainers = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [allTrainersRes, followingRes] = await Promise.all([
        trainerService.getAllTrainers(),
        trainerService.getFollowingTrainers().catch(() => ({ data: { data: [] } }))
      ]);
  ...
  const handleViewProfile = async (trainer) => {
    ...
    try {
      const [detailsRes, coursesRes, blogsRes] = await Promise.all([
        ...
        courseService.getAllCourses({ trainerId: trainer._id }),
        blogService.getBlogsByAuthor(trainer._id)
      ]);
        ...
        {displayedTrainers.length === 0 ? (
          <div className="text-center py-12 bg-white/5 rounded-xl border border-white/10">
            ...
            <p className="text-gray-400">
              {activeTab === 'all' ? 'No trainers available at the moment.' : 'You are not following any trainers yet.'}
            </p>
            {activeTab === 'following' && (
              <button
                onClick={() => setActiveTab('all')}
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="text-gray-400 text-sm mt-1 line-clamp-2">
                    {trainer.bio || 'Fitness trainer helping people achieve their fitness goals.'}
                  </p>
                  
                  <div className="flex items-center gap-4 mt-3 text-sm">
                    <div className="flex items-center gap-1">
                      <VideoCameraIcon className="w-4 h-4 text-purple-400" />
                      <span className="text-gray-300">{trainer.courses || 0} courses</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <DocumentTextIcon className="w-4 h-4 text-blue-400" />
                      <span className="text-gray-300">{trainer.blogs || 0} blogs</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    <span className="text-gray-500 text-xs">({trainer.followers || 0} followers)</span>
                  </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-white/80 text-sm">Fitness Trainer</p>
                    </div>
                  </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center p-3 bg-white/5 rounded-lg">
                    <p className="text-2xl font-bold text-white">{trainerCourses.length}</p>
                    <p className="text-gray-400 text-sm">Courses</p>
                  </div>
                  <div className="text-center p-3 bg-white/5 rounded-lg">
                    <p className="text-2xl font-bold text-white">{trainerBlogs.length}</p>
                    <p className="text-gray-400 text-sm">Blogs</p>
                  </div>
                  <div className="text-center p-3 bg-white/5 rounded-lg">
                    <p className="text-2xl font-bold text-white">{selectedTrainer.followers || 0}</p>
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">
                    {trainerDetails?.bio || 'Certified fitness trainer with expertise in strength training, nutrition, and functional fitness. Dedicated to helping clients achieve their health and wellness goals.'}
                  </p>
                </div>
                ...
                {trainerCourses.length > 0 && (
                  <div>
                    <h3 className="text-white font-semibold mb-3">Courses by {selectedTrainer.name}</h3>
                    <div className="space-y-2">
                      {trainerCourses.slice(0, 3).map((course) => (
                        <div key={course._id} className="bg-white/5 rounded-lg p-3 flex justify-between items-center">
                          <div>
                            <p className="text-white font-medium">{course.title}</p>
                            <p className="text-gray-400 text-xs">{course.enrolledUsers?.length || 0} students</p>
                          </div>
                          <button
                ...
                {/* Blogs */}
                {trainerBlogs.length > 0 && (
                  <div>
                    <h3 className="text-white font-semibold mb-3">Recent Blogs</h3>
                    <div className="space-y-2">
                      {trainerBlogs.slice(0, 3).map((blog) => (
                        <div key={blog._id} className="bg-white/5 rounded-lg p-3">
