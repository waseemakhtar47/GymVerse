...
const GymApplications = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [appsRes, gymRes] = await Promise.all([
        gymService.getGymApplications(gymId),
      ...
      <div className="space-y-6">
        ...
        {applications.length === 0 ? (
          <div className="text-center py-12 bg-white/5 rounded-xl border border-white/10">
            ...
            <p className="text-gray-400">No trainers have applied to this gym yet.</p>
          </div>
        ) : (
                      ...
                      <div className="w-10 h-10 rounded-full bg-linear-to-r from-purple-500 to-blue-500 overflow-hidden">
                        {app.trainerId?.profilePic ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-white font-bold">
                            {app.trainerId?.name?.charAt(0).toUpperCase()}
                          </div>
                        )}
                      </div>
                      <div>
                        <h3 className="text-white font-semibold">{app.trainerId?.name}</h3>
                        <p className="text-gray-400 text-sm">{app.trainerId?.email}</p>
                      </div>
                    </div>
...
export default GymApplications;